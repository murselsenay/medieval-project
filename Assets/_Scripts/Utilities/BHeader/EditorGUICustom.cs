...
public static class EditorGUICustom
{
    ...
    /// <summary>
    /// Custom editor script'lerde BHeader tarzı header çizmek için kullanılır
    /// </summary>
    /// <param name="headerText">Header metni</param>
    /// <param name="isTitle">True ise title style, false ise normal style</param>
    public static void DrawCustomHeader(string headerText, bool isTitle = true)
    {
        EditorGUILayout.Space();

        var indentedRect = EditorGUI.IndentedRect(GUILayoutUtility.GetRect(0, 20));
        Vector2 textSize = EditorStyles.boldLabel.CalcSize(new UnityEngine.GUIContent(headerText));
        Color prevColor = GUI.color;

        Rect headerRect = new Rect(indentedRect.x - 4f, indentedRect.y, indentedRect.width + 8f, textSize.y * 1.3f);
        ...
        if (isTitle)
            GUI.Label(headerRect, headerText, CenteredBigLabel);
        else
        {
            headerRect.x += 8f;
            GUI.Label(headerRect, headerText, MiddleLeftBoldMiniLabel);
        }
