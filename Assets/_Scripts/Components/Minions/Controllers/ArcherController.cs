...
namespace Components.Minions.Controllers
{
    public class ArcherController : MinionController
    {
        ...
        {
            base.Awake();
            _minionType = EMinionType.Archer;
            if (_animator == null)
        ...
        private async UniTaskVoid AttackRoutineRanged(BaseTower target)
        {
            ...
            if (_animator != null)
            {
                ...
                _animator.SetTrigger(EMinionAnimationType.Ranged.ToString());
            }

            ArrowProjectile proj = null;

            proj = await ObjectPool.GetObjectAsync<ArrowProjectile>(_projectileStart ?? transform);

            if (proj == null)
            {
                Debug.LogWarning("ArcherController: ArrowProjectile not found in pool.");
            }
            else
            {
                ...
                proj.Launch(_projectileStart ?? transform, target.transform, _projectileSpeed, null, null, _damage, gameObject.tag, gameObject);

                Debug.DrawLine(startPos, targetPos, Color.yellow, 0.5f);
            }
